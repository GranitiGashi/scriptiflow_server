...
async function uploadBufferAdmin({ buffer, contentType = 'image/png', pathPrefix = 'processed' }) {
  const id = uuidv4();
  const path = `${pathPrefix}/${id}`;
  ...
  if (error) throw error;
  const { data: pub } = await supabaseAdmin.storage.from(BUCKET).getPublicUrl(path);
  return { path, url: pub?.publicUrl };
}

module.exports = { uploadBuffer, uploadBufferAdmin };
